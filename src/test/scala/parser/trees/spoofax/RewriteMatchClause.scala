...
import parser.trees.spoofax.StrategoHelper._
import parser.utils.VarBinder
import org.scalatest.{BeforeAndAfterEach, FunSuite}
import parser.trees.{SpoofaxCanonicalRewriter, SpoofaxTreeBuilder}
...
trait RewriteMatchClause extends BeforeAndAfterEach with NodeChecker {
  ...
  def bindUnnamedVariables(): Unit = {
    test("() => (v_0)") {
      ...
      val rewrite = SpoofaxCanonicalRewriter rewriteTree tree
