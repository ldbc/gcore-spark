...
case class SaveGraph() {
  ...
  def saveGraph(graph: PathPropertyGraph, graph_root_dir: String, hdfs_url: String, parquet: Boolean): Unit =
  {
    val graphName= graph.graphName
    val graphDirectory = graph_root_dir+File.separator+graphName
    ...
    graph.edgeRestrictions.map.foreach(rest => {edge_restrictions = ConnectionRestriction(rest._1.value,rest._2._1.value,rest._2._2.value ):: edge_restrictions})
    ...
    if(parquet){
      graph.vertexData foreach(x => {x.data.asInstanceOf[DataFrame].write.parquet(graphDirectory +File.separator+x.name.value)})
      graph.edgeData foreach(x => {x.data.asInstanceOf[DataFrame].write.parquet(graphDirectory +File.separator+ x.name.value)})
      graph.pathData foreach(x => {x.data.asInstanceOf[DataFrame].write.parquet(graphDirectory +File.separator+ x.name.value)})
    }
    else{
      graph.vertexData foreach(x => {x.data.asInstanceOf[DataFrame].write.json(graphDirectory +File.separator+x.name.value)})
      graph.edgeData foreach(x => {x.data.asInstanceOf[DataFrame].write.json(graphDirectory +File.separator+ x.name.value)})
      graph.pathData foreach(x => {x.data.asInstanceOf[DataFrame].write.json(graphDirectory +File.separator+ x.name.value)})
    }
