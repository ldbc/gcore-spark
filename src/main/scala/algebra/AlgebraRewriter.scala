...
case class AlgebraRewriter(context: AlgebraContext) extends RewriteStage {
  ...
  override def rewrite(tree: AlgebraTreeNode): AlgebraTreeNode = {
    ...
    // Construct rewrite.
    val normalizedBasicConstructs = NormalizeBasicConstructs rewriteTree matchesToAlgebra
    val groupConstructs =
      BasicToGroupConstruct(enrichedContext) rewriteTree normalizedBasicConstructs
